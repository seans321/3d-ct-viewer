// Minimal volume renderer that focuses on getting basic 3D visualization working
class MinimalVolumeRenderer {
    constructor(canvas) {
        this.canvas = canvas;
    ...
    setupShaders() {
        ...
        const fragmentShaderSource = `
            ...
            void main() {
                // If no volume loaded, show a pattern
                if (u_slices <= 0.0) {
                ...
                vec3 step = rayDir * 0.01; // Step size
                ...
                // Ray marching loop
                for (int i = 0; i < 100; i++) {
                    if (accumulatedAlpha >= 0.95 || steps > 100) {
                        break;
                    }
